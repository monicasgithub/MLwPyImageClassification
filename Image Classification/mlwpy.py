...
def np_cartesian_product(*arrays):
    ''' some numpy kung-fu to produce all possible combinations of input arrays '''
    ndim = len(arrays)
...
# solution to this]
def sk_graph_to_tidy(train_test_scores, eval_points, eval_label, num_folds):
    train_scores, test_scores = train_test_scores
    # humph, didn't know np_cartesian was order sensitive
    labels = np_cartesian_product(eval_points, [0,1], np.arange(num_folds))
